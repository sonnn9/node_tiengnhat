!function () {
    ...
    function a(a) {
        var b, c, d, e, f = [];
        for (d = 0; d < a.length; d++)for (e = 0; e < a[d].paths.length; e++)c = Raphael.getTotalLength(a[d].paths[e]), b = {
            "char": d,
            length: c,
    ...
    function d(a) {
        null !== a.object.text && a.object.text.remove(), i.options.stroke.animated.erasing ? (a.object.path.node.style.stroke = i.options.stroke.attr.active, g(a, -1, function () {
            a.object.path.remove(), a.object = {path: null, text: null}
        })) : cb()
    }
    ...
    i.VERSION = "0.2.0", i.options = {
        ...
        prepare: function (d) {
            ...
            if (this.timeouts.length)return !1;
            if ("undefined" == typeof a)a = this.kanjis.length; else if (a > this.kanjis.length)return !1;
            var b, c = function (a) {
    ...
    "use strict";
    function a(a, c, d, e) {
        var f = new XMLHttpRequest, g = ("00000" + d).slice(-5);
        return "00020" === g || "03000" === g ? void e.done(c, {
            paths: [],
            texts: []
        }) : (f.open("GET", a + g + ".svg", !0), f.onreadystatechange = function () {
            4 === f.readyState && (200 === f.status ? e.done(c, b(f.response)) : e.error(f.statusText))
        }, void f.send())
    }
    ...
    function b(a) {
        var b, c = {
            paths: [],
            texts: []
        }, d = (new DOMParser).parseFromString(a, "application/xml"), e = d.querySelectorAll("path"), f = d.querySelectorAll("text");
    ...
    function b(b, d, e) {
        this.element = b, this.text = d, this.options = a.extend({}, e, {element: a(b).attr("id")}), this._name = c, this.dmak = null, this.init()
    }
...
}), $.fn.addSliderSegments = function (a) {
    return this.each(function () {
        var b = 100 / (a - 1) + "%", c = "<div class='ui-slider-segment' style='margin-left: " + b + ";'></div>";
        $(this).prepend(c.repeat(a - 2))
    })
...
var p, options = {
    ...
    width: 109,
    stroke: {
        order: {visible: !1, attr: {fill: "#CDC3C7"}},
        attr: {active: "#E74C3C", stroke: "#7F8C8D", "stroke-width": 4}
    },
    grid: {show: !0},
    loaded: function (a) {
        $(".logo").toggleClass("flip"), $(".wrap-draw").addClass("slideDown"), $slider = $("#slider"), $slider.length > 0 && (p = 1, $slider.slider({
            min: 1,
            max: a.length + 1,
            value: 1,
            ...
            range: "min",
            slide: function (a, b) {
                var c = b.value - p, d = "forward";
                0 > c && (d = "rewind", c = -1 * c), p = b.value;
                for (var e = 0; c > e; e++)$("#draw").dmak(d, 1)
            }
        }).addSliderSegments($slider.slider("option").max))
    },
    erased: function (a) {
        $slider.slider("value") > a && (p = a + 1, $slider.slider("value", p))
    },
    drew: function (a) {
        $slider.slider("value") <= a && (p = a + 1, $slider.slider("value", p))
    }
...
$(function () {
    var a = $(".wrap-draw").html(), b = $("#draw").width();
    if (window.location.hash) {
        var c = window.location.hash.substring(1);
        c = decodeURIComponent(c)
    } else c = "ようこそ！";
    ga("send", "event", "url", "search", c), console.log(b, c.length), b / c.length < options.width && (options.width = Math.floor(b / c.length), options.height = options.width), console.log(options), window.setTimeout(function () {
        $("#draw").dmak(c, options)
    }, 2e3), $("#mybtn").click(function (b) {
        alert('abc')
        b.preventDefault(), $("#draw").data("plugin_dmak") && ($("#draw").dmak("pause"), $(".wrap-draw").html(a));
        var c = $("#mytext").val();
        window.location.hash = "#" + c, ga("send", "event", "input", "search", c), $("#mytext").val(""), $("#draw").dmak(c, options)
    }), $(document).on("click", "#play", function (a) {
        a.preventDefault(), $("#draw").dmak("play")
    }), $(document).on("click", "#pause", function (a) {
        a.preventDefault(), $("#draw").dmak("pause")
    }), $(document).on("click", "#rewind", function (a) {
        a.preventDefault(), $("#draw").dmak("rewind", 1)
    }), $(document).on("click", "#forward", function (a) {
        a.preventDefault(), $("#draw").dmak("forward", 1)
    }), $(document).on("click", "#sample-words span", function (a) {
        a.preventDefault(), $("#mytext").val($(this).html())
    });
    var d = $("#dmak-stroke-width");
    d.slider({
        min: 1,
        max: 9,
        ...
        range: "min",
        stop: function (a, b) {
            options.stroke.attr["stroke-width"] = b.value
        }
    }).addSliderSegments(d.slider("option").max), $("#dmak-color").val(options.stroke.attr.stroke), $(document).on("change", "#dmak-color", function () {
        options.stroke.attr.stroke = $(this).val()
    }), $("#dmak-color-active").val(options.stroke.attr.active), $(document).on("change", "#dmak-color-active", function () {
        options.stroke.attr.active = $(this).val()
    });
    var e = [.1, .05, .015, .009, .005], f = $("#dmak-slider-speed");
    f.slider({
        min: 1, max: 5, value: 3, orientation: "horizontal", range: "min", stop: function (a, b) {
            options.step = e[b.value - 1]
        }
    }).addSliderSegments(f.slider("option").max), $("#dmak-order").html(options.stroke.order.visible ? "ON" : "OFF"), $(document).on("click", "#dmak-order", function (a) {
        a.preventDefault(), "ON" == $(this).html() ? (options.stroke.order.visible = !1, $(this).html("OFF")) : (options.stroke.order.visible = !0, $(this).html("ON"))
    }), $("#dmak-grid").html(options.grid.show ? "ON" : "OFF"), $(document).on("click", "#dmak-grid", function (a) {
        a.preventDefault(), "ON" == $(this).html() ? (options.grid.show = !1, $(this).html("OFF")) : (options.grid.show = !0, $(this).html("ON"))
    })
});